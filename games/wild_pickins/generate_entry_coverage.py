...
def main():
    ...
    p.add_argument('--entry',type=int,choices=(10,15,20),required=True);p.add_argument('--start',type=int,required=True)
    a=p.parse_args()
    if a.rounds<1 or a.start<0:p.error('Invalid count or ID')
    config=json.loads(a.config.read_text());sampler=ConditionalEntry(config['reels']['basegame'],{10:3,15:4,20:5}[a.entry])
    a.output.mkdir(parents=True,exist_ok=False)
    provenance=archive_inputs(a.config.read_bytes(),a.output,a.seed,a.rounds,'generate_entry_coverage.py')
